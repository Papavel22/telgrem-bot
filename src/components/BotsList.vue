<template>
    <div class="container">
      <h1 v-if="bots.length === 0"> You don't have bots</h1>
      <ul v-else class="bot-list">
        <li class="bot-list-title">
          <div class="bot-name">
            Name
          </div>
          <div class="bot-action">actions</div>
        </li>
        <li v-for="bot in bots" :key="bot.id" class="bot-item">
          <h5 v-on:click="onEditBot(bot)" class="bot-title">{{bot.title}}</h5>
          <button v-on:click="onDelBot(bot.id)" class="del-btn">
      ...
      <button v-on:click="openBotModal()" class="btn">add bot</button>
      <BotModal v-on:closeModal="closeModal" v-bind:bot ="editedBot" v-bind:editMode="editMode" v-if="showModal"/>
    </div>
</template>
